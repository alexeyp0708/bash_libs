#!/bin/sh

# Some of the code was borrowed from the NGINX docker image 
# https://github.com/nginxinc/docker-nginx/blob/master/mainline/debian/20-envsubst-on-templates.sh

set -e

ME=$(basename "$0")

templater._log() {
    echo "$@"
}
templater.envsubst() {
    local template_path="$(pwd)/env_tpls"
    local suffix=".tpl"
    local output_path="$(pwd)"
    local filter=""
    local inter_num=1
    
    while getopts "f:o:t:i:h" options; do
        case "${options}" in
            h)
            echo "Replaces variables (all or by filter \"${filter}\") in \"${template_path}/*${suffix}\" files and saves them into files of the same name in the output directory (${output_path})."
            echo "[-f \"env_var|env_var2\"] - Set variables filter"
            echo "[-o \"${output_path}\"]- Set output directory or file for parsed files. Default: STDOUT"
            echo "[-t \"${template_path}\"] - Set templates directory or template file"
            echo "[-i number ] - Default-1. Sets the number of interactions. Repeatedly replaces variables in files "
            echo "----"
            echo "Template file sufix (extension) - ${suffix}"
            echo "Templates directory  -  ${template_path}"
            echo "Output directory  -  ${output_path}"
            echo "Designation of variables in  template file - \${variable}"
            return 0
            ;;
            f)
            filter="${OPTARG}"
            ;;
            t)
            template_path="${OPTARG}"
            ;;
            o)
            output_path="${OPTARG}"
            ;;
            i)
            inter_num="${OPTARG}"
            ;;
        esac
    done
    
    local template defined_envs relative_path output_path subdir random_name random_output_path random_template_path output_path_file file_name
    defined_envs=$(printf '${%s} ' $(awk "END { for (name in ENVIRON) { print ( name ~ /${filter}/ ) ? name : \"\" } }" < /dev/null ))
    #[ -d "$template_path" ] || return 0
    if [ ! -w "$output_path" ]; then
        templater._log "$ME: ERROR: $template_path exists, but $output_path is not writable"
        return 0
    fi
    random_name="$(cat /dev/urandom | tr -dc A-Z-a-z-0-9 | head -n 8 -c 8)"
    random_output_path="$output_path/$random_name"
    random_template_path="$output_path/${random_name}_tpl"
    mkdir -p "${random_output_path}"
    mkdir -p "${random_template_path}" #???
    c=1
    while [ $c -le $inter_num ]
    do
        if [ -d "$template_path" ]
        then
            find "$template_path" -follow -type f -name "*$suffix" -print | while read -r template; do
                relative_path="${template#"$template_path/"}"
                output_path_file="$random_output_path/${relative_path%"$suffix"}"
                subdir=$(dirname "$relative_path")
                # create a subdirectory where the template file exists
                mkdir -p "$random_output_path/$subdir"
                #entrypoint_log "$ME: Running envsubst on $template to $output_path_file"
                envsubst "$defined_envs" < "$template" > "$output_path_file"
            done
        elif [ -f "$template_path" ]
        then
            template=$template_path
            file_name="$(basename ${template})"
            output_path_file="$random_output_path/${file_name%"$suffix"}"
            envsubst "$defined_envs" < "$template" > "$output_path_file"
        fi
        rm -r "$random_template_path"
        mv "$random_output_path" "$random_template_path"
        template_path="$random_template_path"
        suffix=""
        c=$((c+1))
    done
    find "$random_template_path" -maxdepth 1 -type f -name '*' -exec mv -t "$output_path" {} \;
    #mv -t "$output_path" "$random_template_path"/* && mv -t "$output_path" "$random_template_path"/.* 
    rm -r "$random_template_path"
}



if [ -z "$is_packed" ]
then
    "$@"
    exit 0
fi
