...
tester._onCounter(){
    ...
    _counter_test_buf["$name_test"]=0
    _counter_success_buf["$name_test"]=0
    _counter_error_buf["$name_test"]=0
    _counter_empty_test_buf["$name_test"]=0
    ...
    _count_success="${_counter_success_buf["$name_test"]}"
    _count_error="${_counter_error_buf["$name_test"]}"
    _count_tests="${_counter_test_buf["$name_test"]}"
    _count_empty_tests="${_counter_empty_test_buf["$name_test"]}"
}

tester._offCounter(){
    local name_test="$1"
    local current_test=${_active_tests[@]: -1}
    _counter_success_buf["$name_test"]=$(( ${_counter_success_buf["$name_test"]}+$_count_success ))
    _counter_error_buf["$name_test"]=$(( ${_counter_error_buf["$name_test"]}+$_count_error ))
    _counter_test_buf["$name_test"]=$(( ${_counter_test_buf["$name_test"]}+$_count_tests))
    _counter_empty_test_buf["$name_test"]=$(( ${_counter_empty_test_buf["$name_test"]}+$_count_empty_tests ))
    _count_success=${_counter_success_buf["$name_test"]}
    _count_error=${_counter_error_buf["$name_test"]}
    _count_tests=${_counter_test_buf["$name_test"]}
    _count_empty_tests=${_counter_empty_test_buf["$name_test"]}
    if [[ "$(($_count_success+$_count_error))" -eq 0 ]]
    then
            ((++_count_empty_tests))
    fi
}
...
tester.endTest() {
    ...
    _name_current_test="$current_test"
    ...
    if [[ "$(tester.totalOfAsserts "$current_test")" -le 0 ]]
    then
        [ "$disable_display_error" != "yes" ] && echo -e "(Test failure) \033[0;35m Empty \"$current_test\" test ( ${_trace_test["$current_test"]} )\033[0m" >&2
        return $TEST_ERR
    fi
    if [[ $(tester.amountErrors "$current_test") -gt 0 \
    || $(tester.amountEmptyTests "$current_test" ) -gt 0 ]] 
    then
        if [[ "$disable_display_test_error" != "yes" ]]
        then
            echo -e "(Test failure) \033[0;35mBad \"$current_test\" test (${_trace_test["$current_test"]})\033[0m"
        fi
        return $TEST_ERR
    fi
    return 0
...
tester.info(){
    ...
    local amount_success="$(tester.amountSuccess "$name_test")"
    local amount_error="$(tester.amountErrors "$name_test")"
    local amount_tests="$(tester.amountTests  "$name_test")"
    local amount_empty_tests="$(tester.amountEmptyTests  "$name_test")"
    # если имя не указано вывести данные по всем тестам
    echo -e "\033[94m"
    echo "Test $name_test (${_trace_test["$name_test"]})"
    echo "Amount of tests: [ $((amount_tests+1)) ]" 
    echo "Amount of empty tests: [ $amount_empty_tests ]" 
    echo "Amount of assert success: [ $amount_success ]" 
    echo "Amount of assert errors: [ $amount_error ]" 
    echo "Total of asserts: [ $(( $amount_error + $amount_success )) ]"
    echo -en "\033[0m"
}
...
tester.runTest(){
    local status=0
    #local ENTRY_POINT=$(tester._entryPoint)
    shareVar export _counter_success_buf _counter_error_buf _counter_test_buf _counter_empty_test_buf _trace_test _name_core_test _name_current_test disable_display_ok disable_display_error less_message_output _count_success _count_error _count_tests _count_empty_tests _active_tests
